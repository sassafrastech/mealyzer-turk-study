...
class MatchAnswerSummarizer
  ...
  def self.num_matches(answer)
    num_matches = 0
    MatchAnswer.current_study.for_same_component_as(answer).each do |a|
  ...
  def self.build_evaluations_time(answer)
    ...
    ma = MatchAnswer.current_study.where("food_groups = ? AND evaluating_id IS NOT NULL", answer.food_groups.to_json).where("created_at < answer.created_at")
    if !ma.nil?
      ma.each do |a|
         if a.changed_answer
          evals[:incorrect] += 1
